	...
	public boolean playLevel(Command cmd) {
		...
		if(command.equals("")) {
			if(stepCount == 0 && place.equals("Library")) {
				output.setText("Doctor Jones, having recovered both the frog amulet and the snake amulet, has been in the library conducting research into them. After many hours of searching, Doctor Jones stumbles upon a document that might be the answer. [Enter]");
				stepCount++;
			} else if(stepCount == 1 && place.equals("Library")) {
				output.setText("Doctor Jones: Ah ha! Finally! After all this time, I found <i>something</i> pertaining to both amulets. Strange that it was located in this book <b>russianRoyalty.txt</b>. [Enter]");
					stepCount++;
			} else if(stepCount == 2 && place.equals("Library")) {
				output.setText("Doctor Jones: Before I head out, I should lock these amulets up. [Enter]");
				stepCount++;
			} else if(stepCount == 3 && place.equals("Library")) {
				output.setText("After you create a directory, you can alter its permissions so that specific people can do specific commands to it. The Change Mode, <b>chmod</b>, command is used to change permissions. <b>chmod o-rwx [fileName]</b> will prevent <b>[fileName]</b> from being able to be <b>r</b>ead, <b>w</b>ritten, and e<b>x</b>ecuted by <b>o</b>thers. Make a directory and use the <b>chmod</b> command to remove (<b>-</b>) <b>o</b>ther permissions <b>rwx</b>.");
				stepCount++;
			} else if(stepCount == 5 && place.equals("Library")) {
				output.setText("The format for <b>chmod</b> is: \'<b>chmod [ugoa...][+-=][perms...]\'</b> where <b>u</b> stands for <b>u</b>ser, <b>g</b> stands for members of the <b>g</b>roup, and <b>o</b> stands for <b>o</b>thers. Using \'<b>=</b>\' sets the person's permissions exactly to what you specify. In the last command you used, <b>r</b> stood for <b>r</b>eading, <b>w</b> stood for <b>w</b>riting, and <b>x</b> stood for e<b>x</b>ecuting. [Enter]");
				stepCount++;
			} else if(stepCount == 6 && place.equals("Library")) {
				output.setText("Doctor Jones: I better get to the airport before I miss my flight to Russia! I'll bring my <b>permissions.txt</b> so that I have something handy in case I forget <b>chmod</b>!");
				stepCount++;
			} else if(stepCount == 8 && place.equals("Russia")) {
				output.setText("Doctor Jones: Uh-oh, I forgot about the security. There's no way they'll simply let me in! [Enter]");
				stepCount++;
			} else if(stepCount == 9 && place.equals("Russia")) {
				output.setText("Calvin Johnson: \"No permission, no access! That's the law! Now scram! [Enter]");
				stepCount++;
			} else if(stepCount == 10 && place.equals("Russia")) {
				output.setText("Doctor [PLAYERNAME]: No permission, huh? I know a way to change that! [Enter]");
				stepCount++;
				...
				output.setText("Doctor Jones: I did hear about that. I was just on my way to secure another section of the base. [Enter]");
				stepCount++;
				...
				output.setText("Doctor Jones: Whew, that was the worst year of my life. Now where was I? Ah yes, the amulet. Back to the cave.");
				setLocation(red);
				stepCount = 7;
			} else if(stepCount == 7 && place.equals("Russia") && caught) {
				output.setText("Calvin Johnson: Halt! Only those authorized may enter here. Show me your identification! [Enter]");
				caught = false;
				stepCount++;
			} else if (stepCount == 21 && place.equals("Library")) {
				output.setText("Congratulations! You have completed level 4. Hit [Enter] to continue to level 5.");
				return true;
			}
		/* Checking for chmod command */
		} else if(command.equals("chmod")) {
			if(stepCount == 4 && place.equals("Library")) {
				output.setText("Doctor Jones: There, now I am able to access the amulets and no one else is.");
				stepCount++;
			} else if(stepCount == 12 && place.equals("Russia") && cmd.getInputs().get(1).equals("Cave")) {
				output.setText("Doctor Jones: Here is my information, I'm sure that you'll find it satisfactory. [Enter]");
				stepCount++;
			} else if(stepCount == 18 && place.equals("Cave") && !cmd.getInputs().get(0).contains("a")) {
				output.setText("Romeo: Whoooo! I think I heard something! Lock down the door so that no one is able to get it!");
				stepCount++;
			} else if(stepCount == 18 && place.equals("Cave") && cmd.getInputs().get(0).contains("a")) {
				output.setText("Romeo: Aha, it was you! Off to the salt mines with you!");
				caught = true;
				stepCount = 404;
			} else if (stepCount == 20 && place.equals("Box") && cmd.getInputs().get(0).contains("a")) {
				output.setText("Doctor Jones: All of these amulets will make a fine addition to the museum, but how many of them are there? I should return and conduct some more research into them. [Enter]");
				stepCount++;
			}
		/* Checking for cd command */
			...
			if(stepCount == 7 && place.equals("Russia")) {
				...
				output.setText("Romeo: Whoooo are you? There's talk that our sentry outside let someone in without proper clearance. Was it you? [Enter]");
				stepCount++;
			} else if (stepCount == 19 && place.equals("RussianBase")) {
				output.setText("Doctor Jones: “The guard is asleep, I should take this time to look around for that amulet. It’ll be locked up, so I’ll have to set it so that anyone can <b>r</b>ead, <b>w</b>rite, and <b>e</b>xecute it.");
				stepCount++;
			}
		}
